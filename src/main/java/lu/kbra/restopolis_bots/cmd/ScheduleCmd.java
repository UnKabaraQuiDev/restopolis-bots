...
public class ScheduleCmd implements SlashCommandExecutor {
	...
	public void execute(SlashCommandInteractionEvent event) {
		...
		final DiscordPlatformData discordPlatformData = discordPlatformTable.byServer(event.getGuild().getIdLong()).orElse(null);
		if (discordPlatformData == null) {
			event.getHook().sendMessage("Choose your days:").setEphemeral(true).setActionRow(daySelectMenu.build()).queue();
			return;
		}
		final TargetData targetData = targetTable.byId(discordPlatformData.getId());
		event.getHook().sendMessage("Choose your days:").setEphemeral(true).setActionRow(daySelectMenu.build(targetData.getDays())).queue();
	}
