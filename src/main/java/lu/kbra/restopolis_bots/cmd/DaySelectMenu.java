...
public class DaySelectMenu implements DiscordStringMenu, DiscordStringMenuExecutor {
	...
	public void execute(StringSelectInteractionEvent event) {
		...
		DiscordPlatformData discordPlatformData = discordPlatformTable.byServer(event.getGuild().getIdLong()).orElse(null);
		final TargetData targetData;
		if (discordPlatformData == null) {
			targetData = targetTable.insertAndReload(new TargetData(TargetPlatform.DISCORD, Collections.emptyList()));
			discordPlatformData = discordPlatformTable
					.insertAndReload(new DiscordPlatformData(targetData.getId(), event.getGuild().getId(), event.getChannelId(), null));
		} else {
			targetData = targetTable.byId(discordPlatformData.getId());
		}
